...
Usage:
    python run_full_analysis.py                                     # Uses repo default genome
    python run_full_analysis.py path/to/genome.txt                  # Custom genome file
    python run_full_analysis.py --name "Your Name"                # Add name to reports
"""
...
from repo_paths import (DATA_DIR, REPORTS_DIR, DEFAULT_GENOME_HELP,
                        find_default_genome_path, data_file, open_data,
                        ensure_reports_dir)
...
def load_genome(genome_path: Path) -> tuple:
    ...
    with open_data(genome_path) as f:
        for line in f:
            if line.startswith('#'):
                continue
...
def load_pharmgkb() -> dict:
    ...
    annotations_path = data_file("clinical_annotations.tsv")
    alleles_path = data_file("clinical_ann_alleles.tsv")
    ...
    with open_data(annotations_path) as f:
    ...
    with open_data(alleles_path) as f:
        reader = csv.DictReader(f, delimiter='\t')
        for row in reader:
            ann_id = row.get('Clinical Annotation ID', '')
...
def run_full_analysis(genome_path: Path = None, subject_name: str = None):
    ...
    if genome_path is None:
        try:
            genome_path = find_default_genome_path()
        except FileNotFoundError as exc:
            print(f"\nERROR: {exc}")
            sys.exit(1)
    else:
        genome_path = Path(genome_path).expanduser()
...
Examples:
  ...
  python run_full_analysis.py /path/to/genome.txt
  python run_full_analysis.py --name "Your Name"
        """
    )
    parser.add_argument('genome', nargs='?', type=Path, default=None,
                       help=f'Path to 23andMe genome file ({DEFAULT_GENOME_HELP})')
